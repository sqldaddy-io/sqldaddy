...
class PostgreSQL15Structure implements StructureInterface
{

    public function create(string $identifier, string $password): string
    {
        return strtr("CREATE DATABASE fly_database_{identifier}; CREATE USER fly_database_user_{identifier} WITH ENCRYPTED PASSWORD '{password}'; GRANT ALL PRIVILEGES ON DATABASE fly_database_{identifier} TO fly_database_user_{identifier}; ALTER DATABASE fly_database_{identifier} OWNER TO fly_database_user_{identifier}; ALTER USER fly_database_user_{identifier} SET statement_timeout = 30000;  ", [
    ...
    {
        return strtr("DROP DATABASE IF EXISTS fly_database_{identifier}; DROP USER IF EXISTS fly_database_user_{identifier};", [
            '{identifier}' => $identifier
        ]);
    }
